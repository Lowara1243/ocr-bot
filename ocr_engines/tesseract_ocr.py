...
class TesseractOCR(BaseOCR):
    async def recognize(self, image_path: Path, language: Optional[str] = None) -> str:
        try:
            ...
            logger.info(f"Processing image with Tesseract: {image_path}, Language: {lang_param or 'default'}")
            ...
            logger.info(f"Tesseract recognized text (first {OCR_LOG_PREVIEW_LENGTH} chars): {preview_text}")
            return text.strip()
        except pytesseract.TesseractNotFoundError:
            logger.error("Tesseract is not installed or not in your PATH.")
            logger.error("Please install Tesseract and/or set TESSERACT_CMD_PATH in .env")
            return "Error: Tesseract OCR is not configured correctly on the server."
        except RuntimeError as e:
            logger.error(f"Error during Tesseract OCR processing (possibly invalid language '{language}'): {e}")
